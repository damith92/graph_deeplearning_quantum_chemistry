...
def coarsen(A, levels):
    ...
    for i,A in enumerate(graphs):
        ...
        coarsened_graphs.append(A)
        
    return coarsened_graphs, perms[0] if len(perms) > 0 else None
...
def HEM(W, levels, rid=None):
    ...
    parents = []
    #degree = W.sum(axis=0) - W.diagonal() # before
    degree = W.sum(axis=0) # new
    graphs = []
    graphs.append(W)
    ...
    for _ in range(levels):
        ...
        #rid=np.random.permutation(N)                  #  metis/graclus strategy   # new
        ss = np.array(W.sum(axis=0)).squeeze(); rid = np.argsort(ss) # before
...
def perm_data_no_batch(x, indices):
    """
